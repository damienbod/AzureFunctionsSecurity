...
public class CertificateAuthService
{
    ...
    {
        // read from an APP.SETTING is using this
    ...
    private X509Certificate2 GetCertificate(string certificateThumbprint, ILogger log)
    {
        ...
        log.LogInformation("looking for certificate with Thumbprint: {certificateThumbprint}", certificateThumbprint);
        ...
        using (X509Store store = new X509Store(StoreName.My, StoreLocation.CurrentUser))
        {
            ...
            if (certs.Count > 0)
            {
                log.LogInformation("Found certificate: {certificateThumbprint}", certs[0].Thumbprint);
                cert = certs[0];
            }
            store.Close();
        }
